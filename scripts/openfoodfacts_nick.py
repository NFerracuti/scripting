...
class OpenFoodFactsAPI:
    ...
    def search_products(self, category: str, page: int = 1, page_size: int = 50) -> dict:
        ...
        logger.info(f"Searching for {category} products in US/Canada (page {page})")
...
def update_sheet(products_data, is_first_batch=False):
    ...
    try:
        ...
        # First, get existing product codes
        existing_data = sheet.values().get(
            spreadsheetId=CONFIG["spreadsheet_id"],
            range="Sheet1!A:A"  # Updated sheet name
        ).execute()

        existing_codes = set()
        if 'values' in existing_data:
            # Skip header row if it exists
            start_idx = 1 if is_first_batch else 0
            existing_codes = {row[0] for row in existing_data['values'][start_idx:] if row}
        ...
        if is_first_batch:
            ...
            sheet.values().clear(
                spreadsheetId=CONFIG["spreadsheet_id"],
                range="Sheet1!A1:AZ"  # Updated sheet name
            ).execute()
        ...
        if values:  # Only update if we have data to add
            result = sheet.values().append(
                spreadsheetId=CONFIG["spreadsheet_id"],
                range="Sheet1!A1",  # Updated sheet name
...
def main():
    ...
    for category in categories:
        ...
        try:
            # Get first page
            results = api.search_products(category)
            ...
            total_pages = (total_products + 50 - 1) // 50  # Using page_size=50

            logger.info(f"Found {total_pages} total pages. Getting additional pages...")

            # Get remaining pages
            for page in range(2, min(total_pages + 1, 6)):  # Limit to 5 pages for now
                logger.info(f"Getting page {page} of {total_pages}")
                time.sleep(5)  # Add delay between requests

                results = api.search_products(category, page=page)
                if not results or 'products' not in results:
            ...
            if category != categories[-1]:  # Don't wait after last category
                wait_time = 10
                logger.info(f"Waiting {wait_time} seconds before next category...")
